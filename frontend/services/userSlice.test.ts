...
describe("User slice check", () => {
  describe("Update state actions", () => {
    ...
    it("should update only the jwt", () => {
      expect(
        reducer(initialState, actions.update({ jwt: updatedState.jwt }))
      ).toEqual({ ...initialState, jwt: updatedState.jwt });
    });
  ...
  describe("Login state flow", () => {
    it("should set the request state to pending", () => {
      expect(
        reducer(
          { ...initialState, error: { message: "Rejected" } },
    ...
    it("should set the request state to fulfill and reset any previous errors", () => {
      expect(
        reducer(
          { ...initialState, error: { message: "Rejected" } },
          login.fulfilled(
